...
const CVPreview = forwardRef<HTMLDivElement, CVPreviewProps>(({ data, sidebarColor, headingColor, dictionary }, ref) => {
  ...
  return (
    <div ref={ref} className="w-full aspect-[1/1.4142] flex flex-col md:flex-row text-[8px] md:text-[10px] bg-white shadow-lg">
      ...
      <div data-column="left" className={`w-full md:w-1/3 ${sidebarColor} text-white p-4 md:p-6`}>
        ...
        {/* Contact Info */}
        <div className="mb-4">
          <h2 className="text-sm font-bold mb-2">{dictionary.cv.sections.personalInfo}</h2>
        ...
        {data.personalInfo.additionalFields.length > 0 && (
          <div className="mb-4">
            <h2 className="text-sm font-bold mb-2">{dictionary.cv.sections.additionalInfo}</h2>
            <div className="space-y-1.5">
        ...
        {data.skills.length > 0 && (
          <div className="mb-4">
            <h2 className="text-sm font-bold mb-2">{dictionary.cv.sections.skills}</h2>
            <div className="space-y-1.5">
        ...
        {data.languages.length > 0 && (
          <div className="mb-4">
            <h2 className="text-sm font-bold mb-2">{dictionary.cv.sections.languages}</h2>
            <div className="space-y-1.5">
        ...
        {data.interests.length > 0 && (
          <div>
            <h2 className="text-sm font-bold mb-2">{dictionary.cv.sections.interests}</h2>
      ...
      <div data-column="right" className="w-full md:w-2/3 p-4 md:p-6">
        ...
        {data.profile && (
          <section className="mb-6">
            <h2 className={`text-sm font-bold ${headingColor} mb-2`}>{dictionary.cv.sections.profile}</h2>
            <div 
        ...
        {data.education.length > 0 && (
          <section className="mb-6">
            <h2 className={`text-sm font-bold ${headingColor} mb-2`}>{dictionary.cv.sections.education}</h2>
              ...
              {data.education.map((edu) => (
                <div key={edu.id} className="relative pl-3 border-l border-gray-200">
                  ...
                  <p className="text-[7px] text-gray-500">
                    {edu.startDate.month} {edu.startDate.year} - {edu.endDate.current ? dictionary.common.present : `${edu.endDate.month} ${edu.endDate.year}`}
                  </p>
                  {edu.description && (
                    <div 
        ...
        {data.experiences.length > 0 && (
          <section>
            <h2 className={`text-sm font-bold ${headingColor} mb-2`}>{dictionary.cv.sections.experience}</h2>
            <div className="space-y-3">
              {data.experiences.map((exp) => (
                <div key={exp.id} className="relative pl-3 border-l border-gray-200">
                  ...
                  <p className="text-[7px] text-gray-500">
                    {exp.startDate.month} {exp.startDate.year} - {exp.endDate.current ? dictionary.common.present : `${exp.endDate.month} ${exp.endDate.year}`}
                  </p>
                  {exp.description && (
                    <div 
                      className="mt-0.5 text-[8px] text-gray-600"
